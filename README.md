...
| **Filter** | `custom:prism-filter-card` | *Smart* dynamic list: pick a domain + condition (e.g. "lights that are on") and the card fills itself in and re-populates as state changes. Live match count, empty state, inline toggles. |
...
secondary: area      # show each entity's area on the second line
accent: amber
# Other conditions:
